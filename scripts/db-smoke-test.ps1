...
$TestSource = Join-Path $ProjectRoot 'src\test\java\com\kael\punish\DbSmokeTest.java'
...
& $Java -cp $runClasspath com.kael.punish.DbSmokeTest
if ($LASTEXITCODE -ne 0) {
    throw '数据库自检失败'
}

Write-Output 'DbSmokeTest 通过。'
